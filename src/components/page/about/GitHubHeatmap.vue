...
import { useGitHub, type ContributionDay } from '@/hooks/useGitHub'
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div>
        <h3 class="mb-2 text-2xl font-bold">GitHub Activity</h3>
        <p class="text-muted">Consistency and commitment to continuous learning</p>
      </div>

      <div v-if="stats" class="flex gap-6 text-sm">
        <div>
          <div class="text-accent text-2xl font-bold">{{ stats.totalContributions }}</div>
          <div class="text-muted">Total Contributions</div>
        </div>
        <div>
          <div class="text-accent text-2xl font-bold">{{ stats.currentStreak }}</div>
          <div class="text-muted">Current Streak</div>
        </div>
        <div>
          <div class="text-accent text-2xl font-bold">{{ stats.longestStreak }}</div>
    ...
    <!-- Error State -->
    <div v-else-if="error" class="rounded-lg border border-red-500/20 bg-red-500/10 p-6">
      <p class="text-red-500">Failed to load GitHub data: {{ error }}</p>
      <button @click="fetchGitHubStats" class="text-accent mt-2 text-sm hover:underline">
        Try again
      </button>
    </div>
    ...
    <div v-else-if="stats" class="overflow-x-auto pb-4">
      <div class="inline-block min-w-full">
        <!-- Month labels -->
        <div class="text-muted mb-2 flex pl-8 text-xs">
          <div
        ...
        <div class="flex gap-1">
          <!-- Day labels -->
          <div class="text-muted flex flex-col justify-around gap-1 pr-2 text-xs">
            <div>Mon</div>
            <div>Wed</div>
            <div>Fri</div>
          </div>
          ...
          <div class="flex gap-1">
            <div v-for="(week, weekIndex) in weeks" :key="weekIndex" class="flex flex-col gap-1">
              <div
                ...
                class="hover:ring-accent h-3 w-3 cursor-pointer rounded-sm transition-all hover:scale-125 hover:ring-2"
        ...
        <div class="text-muted mt-4 flex items-center gap-2 text-xs">
          <span>Less</span>
          <div class="flex gap-1">
            <div class="bg-muted/30 h-3 w-3 rounded-sm" />
            <div class="bg-accent/30 h-3 w-3 rounded-sm" />
            <div class="bg-accent/50 h-3 w-3 rounded-sm" />
            <div class="bg-accent/70 h-3 w-3 rounded-sm" />
            <div class="bg-accent h-3 w-3 rounded-sm" />
          </div>
          <span>More</span>
        </div>
      </div>
