...
export default function GitHubPinnedRepos({ repos }: Props) {
  ...
  return (
    <div className="rounded-xl border border-border bg-secondary/30 p-5 md:p-6">
      <p className="text-foreground mb-4 text-sm font-semibold">Pinned Repositories</p>
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        {repos.map((repo) => (
          <a
            ...
            className="group flex flex-col gap-2.5 rounded-lg border border-border/60 bg-background/40 p-3.5 no-underline transition-all duration-200 hover:border-accent/40 hover:bg-background/70 hover:shadow-sm"
          >
            {/* Header: name + icon */}
            <div className="flex items-start justify-between gap-2">
              <span className="text-foreground group-hover:text-accent text-sm font-semibold leading-tight transition-colors">
                {repo.name}
              </span>
              <svg
            ...
            {repo.topics.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {repo.topics.map((t) => (
                  <span
                    key={t}
                    className="rounded-full border border-accent/20 bg-accent/8 px-2 py-0.5 font-mono text-[9px] tracking-wide"
                    style={{
